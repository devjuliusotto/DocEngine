...
export default function FormatsPage() {
  return (
    <div className="mx-auto max-w-5xl space-y-8 px-4 py-10 sm:px-6 lg:py-14">
      ...
      <div className="space-y-5">
        {categories.map((category) => {
          const converters = converterRegistry.filter(
            (converter) => converter.category === category.id
          );
          return (
            <EditorialCard key={category.id} className="space-y-5">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/15 pb-4">
                <h2 className="headline text-3xl font-extrabold">{category.title}</h2>
              ...
              <div className="space-y-4">
                {converters.map((converter) => (
                  ...
                  >
                    <h3 className="text-2xl font-extrabold">{converter.name}</h3>
                    <p className="mt-2 text-xl text-white/70">{converter.description}</p>
                    <div className="mt-3 flex flex-wrap gap-2">
                      {[...converter.inputFormats, ...converter.outputFormats].map((format) => (
                        <span
                          key={`${converter.id}-${format}`}
                          className="rounded-full border border-white/15 bg-white/5 px-3 py-1.5 text-base font-extrabold uppercase tracking-[0.04em] text-white"
                        >
                          {format}
                        </span>
                      ))}
                    ...
                    <p className="mt-3 text-lg font-bold">
                      Limite recomendado: {Math.round(converter.maxRecommendedSize / 1024 / 1024)}{" "}
                      MB.{" "}
